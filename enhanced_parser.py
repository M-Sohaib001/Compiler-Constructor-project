...
class SymbolTable:
    ...
    def update(self, name: str, value: Any, symbol_type: str = None) -> bool:
        ...
        if name not in self.symbols:
            # Auto-declare if not exists (for MathScript's implicit declarations)
...
class SemanticAnalyzer:
    ...
    def analyze_assignment(self, variable: str, expr_value: Any, expr_type: str):
        ...
        # Check if variable is being used before declaration
...
class EnhancedParser:
    ...
    def __init__(self, tokens: List[Tuple[str, str]]):
        ...
        self.type_info: Dict[str, str] = {}  # Track types of temps and variables
        self.parse_tree: Optional[ParseTreeNode] = None  # store parse tree after parse()
    ...
    def parse(self) -> Tuple[ParseTreeNode, List[str]]:
        """Parse the input and return parse tree and intermediate code"""
        # Build and store the parse tree so we do not re-parse later
        self.parse_tree = self.statement()
    ...
    def factor(self) -> Tuple[ParseTreeNode, str, str]:
        ...
        if token[0] == 'NUMBER':
            ...
            # Note: In MathScript, variables can be used in expressions after assignment
            symbol = self.symbol_table.lookup(identifier)
            if symbol:
                return factor_node, identifier, symbol.symbol_type
            else:
                # Variable not yet declared - will be if used in assignment
...
def main():
    ...
    try:
        ...
        # Syntax and Semantic Analysis (Part 2)
